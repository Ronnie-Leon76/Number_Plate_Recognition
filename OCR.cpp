...
void OCR::train(Mat TrainData, Mat classes, int nlayers){
    ...
    //Learn classifier
    cv::Ptr<cv::ml::TrainData> tData = cv::ml::TrainData::create(TrainData, cv::ml::ROW_SAMPLE, trainClasses);
    ann->train(tData);
    trained=true;
}
...
int OCR::classifyKnn(Mat f){
    // int response = (int)knnClassifier->findNearest( f, K );
    // return response;
    cv::Mat results, neighborResponses, dists;
    float response = knnClassifier->findNearest(f, K, results, neighborResponses, dists);
